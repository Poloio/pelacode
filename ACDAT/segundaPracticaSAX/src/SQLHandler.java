...
public class SQLHandler {
    ...
    public SQLHandler() throws SQLException {
        ...
        this.connection = SQLConnection.getConnection(); //Obtenemos la conexion a la base de datos
        this.statement = this.connection.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE); //Objeto de tipo statment, los parametros sirven para poder actualizar con resultset
    ...
    public void mostrarListadoEnvios() throws SQLException {
        ...
        this.result = this.statement.executeQuery(sentencia); // Execute query porque devuelve una tabla

        while(this.result.next()) { //Recorremos la tabla

            //Obtenemos los datos de las columnas con los result set
            System.out.println("IDEnvio"+ " "+this.result.getInt("ID") + "  " + "NumContenedores"+" "+this.result.getString("NumeroContenedores") + "  " + "FechaAsignacion"+ "   " + this.result.getString("FechaAsignacion"));
    ...
    private void insertarEnviosEnAsignaciones() throws SQLException {
        ...
        PreparedStatement insertAsignaciones = this.connection.prepareStatement(senteceInsert);

        insertAsignaciones.setInt(1, this.result.getInt("ID")); //Obtenemos el ID (Envio) de la fila en la que se encuentra el result set
        insertAsignaciones.setInt(2, this.result.getInt("AlmacenPreferido")); //Obtenemos el AlmacenPreferido del envio en la fila en la que se encuentra el result set
    ...
    private void insertShipmentsToAsignations(int nuevoAlmacen) throws SQLException {
        ...
        insertAsignaciones.setInt(1, this.result.getInt("ID"));
    ...
    public void asignarEnviosParaAlmacenes() throws SQLException {
        ...
        this.result = this.statement.executeQuery(sentence);

        while(this.result.next()) { //Recorremos con el result set los envios sin asignar
    ...
    private void actualizarEnvio() throws SQLException {
        ...
        this.result.updateDate("FechaAsignacion", java.sql.Date.valueOf(getTodayDate()));
    ...
    private void preguntarNuevoAlmacen() throws SQLException {
        ...
        System.out.println();
        System.out.println("El envio "+this.result.getInt("ID")+" se quedo sin espacio para su almacen preferido "+this.result.getInt("AlmacenPreferido"));
    ...
    private int obtenerAlmacenMasCercanoConEspacio() throws SQLException {
        ...
        CallableStatement almacenMasCercanoConEspacio = this.connection.prepareCall(sentencia); //Calleable statment sirve para llamar a procedimientos almacenamos
        almacenMasCercanoConEspacio.setInt(1, this.result.getInt("AlmacenPreferido")); //Parametro de nuestro procedimiento indicando de que almacen partimos
        almacenMasCercanoConEspacio.setInt(2, this.result.getInt("NumeroContenedores")); //Pasamos el numero de contenedores que tenemos
